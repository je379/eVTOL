...
% Pitch to chord
R.sl = (DF.m - (1 - (V.rel2 ./ V.rel1))) ./ (V.theta2 ./ (2.*V.rel1));
S.sl = (DF.m - (1 - (V.x ./ V.abs2))) ./ (V.theta2 ./ (2.*V.abs2));

% Metal angles
angle.chi1 = abs(angle.b1)+angle.i1;
angle.chi2 = (abs(angle.b2) - abs(angle.chi1) .* R.m .* R.sl .^ (0.5)) ./ (1 - R.m.*R.sl.^(0.5));
angle.chi3 = angle.a2+angle.i3;
angle.chi4 = (abs(angle.a3) - abs(angle.chi3) .* S.m .* S.sl .^ (0.5)) ./ (1 - S.m.*S.sl.^(0.5));
