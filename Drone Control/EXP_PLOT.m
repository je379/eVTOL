...
plot(S12.Re, S12.FOM, 'xb');
plot(S08L.Re, S08L.FOM, '-xr');
plot(S10L.Re, S10L.FOM, '-xk');
plot(S12L.Re, S12L.FOM, '-xb');
plot([min(S08L.Re) max(S12L.Re)], [mean(BASE.FOM) mean(BASE.FOM)], '-.g');
plot([min(S08L.Re) max(S12L.Re)], [mean(APC.FOM) mean(APC.FOM)], '-g');
...
ylabel('Thrust / N'); 
ylim([0 max([max(S08L.T), max(S10L.T), max(S12L.T), max(APC.T), max(BASE.T), max(S12.T)])]);
xlim([0 max([max(S08L.P), max(S10L.P), max(S12L.P), max(APC.P), max(BASE.P), max(S12.P)])]);
% plot(S08.rpmmean, S08.FOM, '-or');
% plot(S10.rpmmean, S10.FOM, '-ok');
plot(S12.P, S12.T, 'xb');
...
plot(APC.P, APC.T, '-g');
% plot(APC.Re, APC.FOM, 'xg');
legend('Sigma = 0.8', 'Sigma = 1.0', 'Sigma = 1.2', 'Baseline Propellor', 'APC Propellor', 'Location', 'southeast');
% legend('SHORT Sigma = 0.8', 'SHORT Sigma = 1.0', 'SHORT Sigma = 1.2', 'LONG Sigma = 0.8', 'LONG Sigma = 1.0', 'LONG Sigma = 1.2', 'Baseline Propellor', 'APC Propellor', 'Location', 'east');
